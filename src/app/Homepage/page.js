...
import { Calculator, FlaskConical, PenTool, Palette, BookOpen, User, Settings, Volume2, Earth } from "lucide-react"
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-6xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-slate-800 mb-2">Welcome back, Charlie! 👋</h1>
            <p className="text-xl text-slate-600">Ready to learn something new today?</p>
          </div>
                ...
                <Link href={subject.path} key={subject.id}>
                    ...
                    <CardContent className="p-8 text-center">
                        ...
                        <div className="flex flex-col items-center gap-6">
                            ...
                            <IconComponent className={`h-16 w-16 ${subject.iconColor}`} strokeWidth={2} />
                            </div>
                            <h2 className="text-3xl font-bold text-slate-800">{subject.title}</h2>
                        </div>
                        </button>
                    </CardContent>
                    </Card>
        ...
        <footer className="mt-12 text-center">
          <p className="text-lg text-slate-500">Take your time and have fun learning! 🌟</p>
        </footer>
      </div>
    </div>
  )
